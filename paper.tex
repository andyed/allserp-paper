...
Population: 2,775 trials processed (1 trial dropped: missing meta or fixations); 37,142 typed AOI rows under the gap-fill flavor. Table~\ref{tab:inventory} reports the per-element-type behavioral inventory; column denominators differ~--- see notes below.
...
\begin{figure*}[t]
  ...
  \Description[Two bar charts comparing click rate by position under two AOI-flavor conventions]{Two side-by-side bar charts of click rate by SERP position 0 to 9. The left chart, organic-only flavor in purple, shows position 0 at 39.5 percent declining steeply to 4 to 8 percent at positions 7 plus. The right chart, organic-hybrid flavor in teal, shows position 0 at 19.9 percent (highlighted in orange to mark dd\_top dominance), with the click peak at position 1 at 25.6 percent and gradual decline thereafter.}
  \caption{Click rate by position under the two main flavors AllSERP releases. \textbf{Left: organic-only flavor}~--- position 0 is the topmost organic and the click heavyweight (39.5\,\%); ranks decline to 4--8\,\% by position 7+ (Spearman $\rho = -0.624$). \textbf{Right: organic-hybrid flavor}~--- position 0 is the topmost main-axis card, often a \texttt{dd\_top} ad (orange bar; 19.9\,\%); the click peak shifts to position 1 (25.6\,\%), where the top organic now sits ($\rho = -0.939$). Two flavors, two stories: consumers should pick the one that matches their question (``what does the top organic earn?'' vs ``what does the topmost SERP slot earn?'').}
  \label{fig:rank}
\end{figure*}

\section{How to Use}\label{sec:howto}

The release is a corpus CSV (per-AOI geometry and etype) plus per-trial JSONs. The cursor-approach feature file (per-AOI dwell, click outcome, etype) is the join target for click-rate work. A worked example using the corpus CSV for an above-fold organic count and the cursor-approach file for per-position click rate:

\begin{verbatim}
import json, polars as pl
# Per-AOI geometry + etype + organic_rank.
aois = pl.read_csv(
  "adserp_aois_by_trial_id_typed_gapfill.csv")
...
caf = pl.read_json(
  "cursor-approach-features-typed-gapfill.json")
...
\begin{itemize}
  ...
  \item \textbf{Derived data (CC-BY-4.0).} Per-trial gap-fill AOI JSONs ($\sim$19~MB across 2,776 trials), corpus CSV (37,142 rows), per-(trial, position) content-feature file (lexical, query-overlap, semantic-cosine~--- see repo README for the methodology and a snippet-TTR rank-confound caveat that consumers must partial out before per-position content claims).
  ...
  \item \textbf{Underlying corpus.} AllSERP requires the AdSERP Zenodo volume (\url{https://zenodo.org/records/15236546})~--- AdSERP authors release it CC-BY-4.0; AllSERP does not redistribute screenshots or HTML.
\end{itemize}

\subsection{Citation}

AllSERP is a derived release built on the AdSERP corpus. Work that uses AllSERP should cite both papers~--- AllSERP for the typed AOI extraction and per-element behavioral enrichment, AdSERP for the underlying multimodal signals.
...
Use of only the typed AOI labels and the extraction pipeline (e.g., applying the methodology to a different SERP corpus) requires only the AllSERP citation; any use of the underlying gaze, cursor, scroll, pupil, or screenshot signals additionally requires the AdSERP citation.
