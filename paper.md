...
The AdSERP dataset \cite{latifzadeh2025adserp} occupies a distinctive position in the multimodal IR resource landscape: simultaneous eye gaze, cursor, scroll, pupil, and click telemetry against ground-truth screenshots, on real Google search-result pages with realistic commercial-intent task framing. The dataset's authors framed their contribution as the first publicly available large-scale resource combining (1) eye-tracking, (2) mouse trajectories, and (3) SERP HTML amenable to programmatic AOI segmentation at thousands-of-trials scale, with eye fixations on programmatically extracted AOIs intended as ground-truth labels for inferring visual attention from mouse movements at high granularity. They additionally noted that their release revisits prior research findings on user SERP interaction and discusses new insights derived from the dataset, with full release on publication. AdSERP supplies the substrate; the per-element AOI extraction implied by that framing was left for downstream work, and follow-up work has begun building on it — most notably the paired SIGIR '25 release on multi-slot ad attention quantification \cite{villaizan2025adsight} and pupillometric methods work that uses AdSERP gaze data as a validation context \cite{jayawardena2025ripa2}. This paper realizes the per-element AOI extraction at the geometry layer.
...
This paper was drafted in collaboration with Anthropic's Claude (Opus 4.7). Claude assisted with drafting prose, structuring sections, cross-checking citations against the bibliography, and surfacing inconsistencies between draft text and upstream code outputs. All quantitative claims, citations, and methodological decisions were verified by the authors against the executable scripts and per-trial JSONs released with the upstream repositories. The authors take full responsibility for the paper's content.
